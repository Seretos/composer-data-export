...
class JsonExportCommandTest extends PHPUnit_Framework_TestCase{
	...
	public function configure_method(){
		...
		$this->assertSame('output file for composer data json result', $definition->getArguments()['output']->getDescription());
		...
		$this->assertSame('export the informations about the current composer project in an json format', $this->command->getDescription());
	}
	...
	public function execute_method(){
		...
		$mockService = $this->getMockBuilder(ExportService::class)->disableOriginalConstructor()->getMock();
		$mockFactory = $this->getMockBuilder(ExportFactory::class)->disableOriginalConstructor()->getMock();
		
		$mockInput->expects($this->at(0))->method('getArgument')->with('output')->will($this->returnValue('my.file'));
		
		$this->mockContainer->expects($this->at(0))->method('get')->with('json.export.service')->will($this->returnValue($mockService));
		$this->mockContainer->expects($this->at(1))->method('get')->with('json.export.factory')->will($this->returnValue($mockFactory));

		$mockService->expects($this->at(0))->method('load')->with($this->mockComposer);
		$mockService->expects($this->at(1))->method('createJsonArray')->with($this->mockComposer)->will($this->returnValue(['success']));
		
		$mockFactory->expects($this->at(0))->method('saveJsonFile')->with('my.file',['success']);
